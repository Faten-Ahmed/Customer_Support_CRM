...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, MatDialogModule, MatFormFieldModule, MatSelectModule, MatInputModule, MatButtonModule, TranslatePipe],
  template: `
    <h2 mat-dialog-title>{{ 'modal.changeStatusTitle' | translate }}</h2>
    <mat-dialog-content>
      <form [formGroup]="form" class="flex flex-col gap-3 pt-2">
        <mat-form-field appearance="outline">
          <mat-label>{{ 'modal.newStatus' | translate }}</mat-label>
        ...
        @if (form.get('status')?.value === 'Resolved') {
          <mat-form-field appearance="outline">
            <mat-label>{{ 'modal.resolutionNotes' | translate }}</mat-label>
    ...
    <mat-dialog-actions align="end">
      ...
      <button mat-flat-button color="primary" [disabled]="form.invalid" (click)="onSubmit()">{{ 'modal.confirm' | translate }}</button>
    </mat-dialog-actions>
  `,
})
